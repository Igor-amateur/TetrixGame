...
{
	SMALL_RECT windowSize = { 0, 0, static_cast<SHORT>(X_SIZE), static_cast<SHORT>(Y_SIZE) };
	COORD windowBufSize = { static_cast<SHORT>(X_SIZE + 1), static_cast<SHORT>(Y_SIZE + 1) };
	...
	if (mConsole != INVALID_HANDLE_VALUE)
	{
		...
		SetConsoleCursorInfo(mConsole, &ccf); //Set new cursor properties
